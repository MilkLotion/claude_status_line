...
// 활성화(켜기)만 특별히 엄격하게 — 오탐으로 제멋대로 켜지는 것을 막는다.
// (1) "제한/한도/캡" 단어(LIMIT_RE)를 필수로 하고 (기존엔 선택이라 "사용량 80"도 걸림)
// (2) 활성화 동사(걸어/설정/켜/적용 등)까지 함께 있을 때만 매칭한다.
// → "사용량이 80%네" 같은 관찰성 발화나 "사용량 제한 80%"만으로는 켜지지 않고,
//    "사용량 제한 80% 걸어" 처럼 명시적 지시일 때만 켜진다.
const ENABLE_VERBS =
  "걸어|걸게|걸어줘|걸어줄래|설정|활성|켜|켜줘|적용|enable|on|set";
// 제한 단어와 임계 숫자 사이 간격 — GAP과 달리 숫자를 포함하지 않는다.
// (GAP은 [^.!?\n] 이라 숫자도 먹어 "80"의 "8"까지 삼키고 threshold가 0으로 잘린다)
const NUM_GAP = "[^0-9.!?\\n]{0,12}";
const ENABLE_RE = new RegExp(
  `(?:${USAGE_RE})${GAP}(?:${LIMIT_RE})${NUM_GAP}(\\d+)\\s*%?${GAP}(?:${ENABLE_VERBS})`,
  "i",
);
