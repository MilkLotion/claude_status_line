...
> **Claude Code로 자동 설치**: 아래 수동 설치 대신, clone 받은 이 디렉토리에서 Claude Code 세션을 열고 "전역세팅해줘" 라고 말하면 `CLAUDE.md`에 적힌 절차대로 OS(윈도우/맥/리눅스)를 구분해서 `statusline.py` 복사 + 훅 설치 + `settings.json` 병합까지 자동으로 진행됩니다.
